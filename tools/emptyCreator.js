            ...
            folderSchema.updateMany(
                ...
                function (err, result) {
                    if (err) {
                        console.log(err);
                        if (!isFolder) {
                            fileSchema.findByIdAndDelete(item._id).then(() => res());
                        }
                        else{
                            res();
                        }
            ...
            tool.neighbourNames(pathString, 2)
                .then((names) => {
                    ...
                    for (let i = 0; i < req.body.length; i++) {
                        ...
                        if (item.type === 'folder' && names[0].includes(itemName + item.extension) === false) {
                        ...
                        else if (item.type === 'file' && names[1].includes(itemName + item.extension) === false) {
